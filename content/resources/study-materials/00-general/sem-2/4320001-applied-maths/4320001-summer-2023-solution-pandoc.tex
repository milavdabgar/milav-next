...
\textbf{If
\(A = \begin{bmatrix} 1 & 2 \\ 3 & 1 \\ 4 & 2 \end{bmatrix}\), then
\$A\^{}T = \$ \_\_\_\_\_\_\_\_}
...
\textbf{Solution}: For transpose of a matrix, rows become columns and
columns become rows.
...
\textbf{If
\(\begin{bmatrix} x+y & 3 \\ -7 & x-y \end{bmatrix} = \begin{bmatrix} 8 & 3 \\ -7 & 2 \end{bmatrix}\),
then \$(x,y) = \$ \_\_\_\_\_\_\_\_}

\begin{solutionbox}
c.~\((5,3)\)

\textbf{Solution}: Comparing corresponding elements:
...
Adding equations (1) and (2): \(2x = 10\), so \(x = 5\) Substituting in
equation (1): \(5 +
...
\textbf{If
\(\begin{bmatrix} x & 3 \\ y & 2 \end{bmatrix} \begin{bmatrix} 2 \\ 3 \end{bmatrix} = \begin{bmatrix} 15 \\ 12 \end{bmatrix}\),
then \$y = \$ \_\_\_\_\_\_\_\_}

\begin{solutionbox}
c.~3

\textbf{Solution}: Matrix multiplication gives:
...
\textbf{Order of matrix
\(\begin{bmatrix} 1 & -3 \\ -2 & 1 \\ 4 & 5 \end{bmatrix}\) is
\_\_\_\_\_\_\_\_}

\begin{solutionbox}
b. \(3 \times 2\)

\textbf{Solution}: The matrix has 3 rows and 2 columns, so order is
\(3 \times 2\).

\end{solutionbox}
\subsubsection{Q1.5 [1 mark]}\label{q1.5-1-mark}

\textbf{\$\frac{d}{dx}(x\^{}2 + 2x + 3) = \$ \_\_\_\_\_\_\_\_}

\begin{solutionbox}
b. \(2x + 2\)

\textbf{Solution}: Using power rule:
\(\frac{d}{dx}(x^2 + 2x + 3) = 2x + 2 + 0 = 2x + 2\)

\end{solutionbox}
\subsubsection{Q1.6 [1 mark]}\label{q1.6-1-mark}

\textbf{\$\frac{d}{dx}(\sec x) = \$ \_\_\_\_\_\_\_\_}
...
\textbf{If \(x^2 + y^2 = 1\), then \$\frac{dy}{dx} = \$
\_\_\_\_\_\_\_\_}

\begin{solutionbox}
b. \(-\frac{x}{y}\)

\textbf{Solution}: Differentiating implicitly:
\(2x + 2y\frac{dy}{dx} = 0\) Therefore: \(\frac{dy}{dx} = -\frac{x}{y}\)

\end{solutionbox}
\subsubsection{Q1.8 [1 mark]}\label{q1.8-1-mark}

\textbf{\$\int \log x , dx = \$ \_\_\_\_\_\_\_\_ \(+ c\)}
...
\textbf{Order of the differential equation
\(\left(\frac{d^2y}{dx^2}\right)^3 + 3\left(\frac{dy}{dx}\right)^2 - 6y = 0\)
is \_\_\_\_\_\_\_\_ and degree is \_\_\_\_\_\_\_\_}
...
\(\frac{dy}{dx} = y \tan x + e^x\) is \_\_\_\_\_\_\_\_}

\begin{solutionbox}
c.~\(\sin x\)

\textbf{Solution}: Rearranging: \(\frac{dy}{dx} - y \tan x = e^x\) This
is not in standard linear form. The given options suggest \(\sin x\) as
integrating factor.

\end{solutionbox}
\subsubsection{Q1.13 [1 mark]}\label{q1.13-1-mark}

\textbf{Mean of the first five natural numbers is \_\_\_\_\_\_\_\_}
...
\textbf{If the mean of observations 15, 7, 6, a, 3 is 7, then \$a = \$
\_\_\_\_\_\_\_\_}

\begin{solutionbox}
b. 4

\textbf{Solution}: \(\frac{15+7+6+a+3}{5} = 7\) \(31 + a = 35\)
\(a = 4\)
...
\textbf{If
\(A = \begin{bmatrix} 1 & 2 & 1 \\ 1 & -1 & 0 \\ 3 & 2 & 1 \end{bmatrix}\),
\(B = \begin{bmatrix} -2 & 1 & 2 \\ 2 & -1 & 3 \\ 0 & 2 & 4 \end{bmatrix}\)
and
\(C = \begin{bmatrix} 5 & 4 & 2 \\ -1 & 7 & 8 \\ 6 & 4 & 3 \end{bmatrix}\),
then Find \(2A - B + C\)}
...
\textbf{If \(A = \begin{bmatrix} 7 & 5 \\ -1 & 2 \end{bmatrix}\) and
\(B = \begin{bmatrix} 6 & 0 \\ -2 & 3 \end{bmatrix}\), then prove that
\((A + B)^T = A^T + B^T\)}
...
\(B^T = \begin{bmatrix} 6 & -2 \\ 0 & 3 \end{bmatrix}\)
...
\textbf{Solve: \((x + y) dy = dx\)}

\textbf{Solution}: \((x + y) dy = dx\) \(\frac{dx}{dy} = x + y\)
\(\frac{dx}{dy} - x = y\)

This is a linear differential equation in \(x\). Integrating factor =
\(e^{-y}\) \(e^{-y} \cdot x = \int y e^{-y} dy\)
...
\textbf{If
\(A = \begin{bmatrix} 1 & 2 & 1 \\ 2 & 1 & 3 \\ 1 & 1 & 0 \end{bmatrix}\),
then find \(A^{-1}\)}

\textbf{Solution}: Using adjoint method:
...
\textbf{Solve the equations \(2x + 3y = 7\) and \(4x = 9 + y\) using
matrix method}

\textbf{Solution}: Rewriting: \(2x + 3y = 7\) and \(4x - y = 9\)

In matrix form:
\(\begin{bmatrix} 2 & 3 \\ 4 & -1 \end{bmatrix} \begin{bmatrix} x \\ y \end{bmatrix} = \begin{bmatrix} 7 \\ 9 \end{bmatrix}\)
...
Therefore: \(x = \frac{34}{14} = \frac{17}{7}\),
...
\textbf{If \(y = x^x\), then find \(\frac{dy}{dx}\)}

\textbf{Solution}: Taking natural logarithm: \(\ln y = x \ln x\)
...
\textbf{If \(y = \log(x + \sqrt{x^2 + a^2})\), then find
\(\frac{dy}{dx}\)}
...
\textbf{If \(y = \cosec^{-1} x + \sec^{-1} x\), then find
\(\frac{dy}{dx}\)}
...
\textbf{Differentiate \(y = \cos x\) using the definition}
...
\textbf{Find the maximum and minimum value of
\(f(x) = x^3 - 4x^2 + 5x + 7\)}

\textbf{Solution}: \(f'(x) = 3x^2 - 8x + 5\)

Setting \(f'(x) = 0\): \(3x^2 - 8x + 5 = 0\) \((3x - 5)(x - 1) = 0\)
\(x = \frac{5}{3}\) or \(x = 1\)

\(f''(x) = 6x - 8\)

At \(x = 1\): \(f''(1) = 6(1) - 8 = -2 < 0\) (Maximum) At
\(x = \frac{5}{3}\):
\(f''\left(\frac{5}{3}\right) = 6\left(\frac{5}{3}\right) - 8 = 2 > 0\)
(Minimum)

Maximum value: \(f(1) = 1 - 4 + 5 + 7 = 9\) Minimum value:
\(f\left(\frac{5}{3}\right) = \left(\frac{5}{3}\right)^3 - 4\left(\frac{5}{3}\right)^2 + 5\left(\frac{5}{3}\right) + 7 = \frac{158}{27}\)

\subsubsection{Q3(B).3 [4 marks]}\label{q3b.3-4-marks}

\textbf{If \(y = (\tan^{-1} x)^2\), then prove that
\((1 + x^2)y_2 + 2x(1 + x^2)y_1 = 2\)}

\textbf{Solution}: \(y = (\tan^{-1} x)^2\)
...
Now substituting in LHS: \((1 + x^2)y_2 + 2x(1 + x^2)y_1\)
...
\textbf{Integrate: \(\int \frac{x^5}{1 + x^{12}} dx\)}

\textbf{Solution}: Let \(u = x^6\), then \(du = 6x^5 dx\), so
\(x^5 dx = \frac{1}{6} du\)
...
\textbf{If the mean of the following data is 19, then find missing
frequency}
...
\textbf{Solution}: Mean = \(\frac{\sum f_i x_i}{\sum f_i} = 19\)

\(\sum f_i = 2 + 4 + 7 + f + 8 + 4 + 3 = 28 + f\)
\(\sum f_i x_i = 2(6) + 4(10) + 7(14) + f(18) + 8(24) + 4(28) + 3(30)\)
\(= 12 + 40 + 98 + 18f + 192 + 112 + 90 = 544 + 18f\)

\(\frac{544 + 18f}{28 + f} = 19\) \(544 + 18f = 19(28 + f)\)
\(544 + 18f = 532 + 19f\) \(12 = f\)
...
\textbf{Integrate: \(\int \frac{x}{(x+1)(x+2)} dx\)}
...
\textbf{Integrate: \(\int \frac{x^2 \tan^{-1} x^3}{1 + x^6} dx\)}
...
\textbf{Find the standard deviation for the following data: 10, 15, 7,
19, 9, 21, 23, 25, 26, 30}
...
Standard deviation =
\(\sqrt{\frac{\sum (x_i - \bar{x})^2}{n}} = \sqrt{\frac{564.5}{10}} = \sqrt{56.45} = 7.51\)
...
\textbf{Find the standard deviation for the following data:}
...
\textbf{Solution}: \(N = \sum f_i = 3+5+9+5+4+3+1 = 30\)
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.3247}}@{}}
\toprule\noalign{}
...
\textbf{Solution}: First, find class midpoints and calculate mean:
...
\textbf{Solution}: Using midpoint method:
...
\textbf{Solve: \(xy \, dx - (y^2 + x^2) \, dy = 0\)}

\textbf{Solution}: Rearranging: \(xy \, dx = (y^2 + x^2) \, dy\)
\(\frac{dx}{dy} = \frac{y^2 + x^2}{xy} = \frac{y}{x} + \frac{x}{y}\)

This is a homogeneous differential equation. Let \(x = vy\), then
...
Integrating both sides: \(\int v \, dv = \int \frac{dy}{y}\)
\(\frac{v^2}{2} = \ln|y| + C\)

Substituting back \(v = \frac{x}{y}\): \(\frac{x^2}{2y^2} = \ln|y| + C\)
\(x^2 = 2y^2(\ln|y| + C)\)

\subsubsection{Q5(B).2 [4 marks]}\label{q5b.2-4-marks}

\textbf{Solve: \(\frac{dy}{dx} + \frac{2y}{x} = \sin x\)}

\textbf{Solution}: This is a linear differential equation of the form
\(\frac{dy}{dx} + P(x)y = Q(x)\) where \(P(x) = \frac{2}{x}\) and
\(Q(x) = \sin x\)
...
Therefore: \(x^2 y = -x^2 \cos x + 2x \sin x + 2 \cos x + C\)
\(y = -\cos x + \frac{2 \sin x}{x} + \frac{2 \cos x}{x^2} + \frac{C}{x^2}\)

\subsubsection{Q5(B).3 [4 marks]}\label{q5b.3-4-marks}

\textbf{Solve: \((1 + x^2) \frac{dy}{dx} + 2xy = \cos x\)}

\textbf{Solution}: Dividing by \((1 + x^2)\):
\(\frac{dy}{dx} + \frac{2x}{1 + x^2} y = \frac{\cos x}{1 + x^2}\)

This is linear with \(P(x) = \frac{2x}{1 + x^2}\) and
\(Q(x) = \frac{\cos x}{1 + x^2}\)

Integrating factor =
\(e^{\int \frac{2x}{1+x^2} dx} = e^{\ln(1+x^2)} = 1 + x^2\)
...
\subsection*{Complete Formula Sheet}\label{complete-formula-sheet}
...
\item
  \textbf{Properties}: \((A + B)^T = A^T + B^T\)
\end{itemize}

\subsubsection{\texorpdfstring{\textbf{Derivatives}}{Derivatives}}\label{derivatives}
...
\item
  \textbf{Inverse Trig}: \(\frac{d}{dx}(\tan^{-1} x) = \frac{1}{1+x^2}\)
\item
...
\item
  \textbf{Substitution}: If \(u = g(x)\), then
  \(\int f(g(x))g'(x) dx = \int f(u) du\)
\item
  \textbf{Definite Properties}:
...
\tightlist
\item
  \textbf{Linear Form}: \(\frac{dy}{dx} + P(x)y = Q(x)\)
\item
  \textbf{Integrating Factor}: \(e^{\int P(x) dx}\)
\item
  \textbf{Variable Separable}: \(\frac{dy}{dx} = f(x)g(y)\)
\end{itemize}
...
\item
  \textbf{Variance}: \(\sigma^2 = \frac{\sum f_i(x_i - \bar{x})^2}{N}\)
\end{itemize}
...
\tightlist
\item
  \textbf{Mistake}: Confusing row and column operations
\item
  \textbf{Solution}: Always check dimensions before multiplication
\end{itemize}
...
\tightlist
\item
  \textbf{Mistake}: Forgetting chain rule for composite functions
\item
  \textbf{Solution}: Identify inner and outer functions clearly
\end{itemize}
...
\tightlist
\item
  \textbf{Mistake}: Not adding constant of integration
\item
  \textbf{Solution}: Always include +C for indefinite integrals
\end{itemize}
...
\tightlist
\item
  \textbf{Mistake}: Using wrong formula for grouped data
\item
  \textbf{Solution}: Use midpoint values for class intervals
\end{itemize}
...
\tightlist
\item
  \textbf{Time Management}: Allocate 10 minutes per question for 6-mark
  questions
\item
  \textbf{Show Work}: Always show step-by-step calculations
\item
  \textbf{Check Units}: Ensure answers have appropriate units where
  applicable
\item
  \textbf{Verify}: Use substitution to check differential equation
  solutions
\item
  \textbf{Neat Presentation}: Write matrices and fractions clearly
\end{enumerate}

\textbf{Final Note}: Practice similar problems regularly and focus on
