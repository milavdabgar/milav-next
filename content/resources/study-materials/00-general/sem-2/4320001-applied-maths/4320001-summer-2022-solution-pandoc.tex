...
\(A^2\) = \ldots\ldots\ldots{}}

\begin{solutionbox}
(c) \(\begin{bmatrix} 7 & 15 \\ 22 & 10 \end{bmatrix}\)
...
\textbf{If \(A = \begin{bmatrix} 1 & 3 \\ 4 & -2 \end{bmatrix}\) then
\(2A - 2I\) = \ldots\ldots\ldots{}}

\begin{solutionbox}
(a) \(\begin{bmatrix} 0 & 6 \\ -8 & -6 \end{bmatrix}\)
...
\textbf{If \(A = \begin{bmatrix} -8 & -6 \\ 3 & 4 \end{bmatrix}\) then
\(\text{Adj } A\) = \ldots\ldots\ldots{}}
...
\(\begin{bmatrix} 5 & 2 & 20 & 41 & 0 \\ 15 & 4 & 30 & 40 & 1 \\ 25 & 6 & 40 & 39 & 2 \\ 35 & 8 & 50 & 38 & 3 \end{bmatrix}\)
is \ldots\ldots\ldots{}}

\begin{solutionbox}
(b) \(4 \times 5\)

\textbf{Solution}: The matrix has 4 rows and 5 columns, so the order is
\(4 \times 5\).

\end{solutionbox}
\subsubsection{Q1.5 [1 mark]}\label{q1.5-1-mark}

\textbf{\(\frac{d}{dx}(\cos^2 x + \sin^2 x)\) = \ldots\ldots\ldots{}}

\begin{solutionbox}
(d) 0

\textbf{Solution}: Since \(\cos^2 x + \sin^2 x = 1\) (trigonometric
identity) \(\frac{d}{dx}(1) = 0\)

\end{solutionbox}
\subsubsection{Q1.6 [1 mark]}\label{q1.6-1-mark}

\textbf{If \(f(x) = \log x\) then \(f'(1)\) = \ldots\ldots\ldots{}}

\begin{solutionbox}
(a) 1

\textbf{Solution}: \(f(x) = \log x\) \(f'(x) = \frac{1}{x}\)
\(f'(1) = \frac{1}{1} = 1\)

\end{solutionbox}
\subsubsection{Q1.7 [1 mark]}\label{q1.7-1-mark}

\textbf{If \(x^2 + y^2 = a^2\) then \(\frac{dy}{dx}\) =
\ldots\ldots\ldots{}}
...
\textbf{\(\int e^{x\log a} dx\) = \ldots\ldots..}

\begin{solutionbox}
(d) \(\frac{a^x}{\log a}\)

\textbf{Solution}: \(e^{x\log a} = a^x\)
\(\int a^x dx = \frac{a^x}{\log a} + c\)

\end{solutionbox}
\subsubsection{Q1.10 [1 mark]}\label{q1.10-1-mark}

\textbf{\(\int \cot x dx\) = \ldots\ldots..}

\begin{solutionbox}
(a) \(\log|\sin x|\)

\textbf{Solution}: \(\int \cot x dx = \int \frac{\cos x}{\sin x} dx\)
...
\textbf{Order of differential equation
\(\left(\frac{d^2y}{dx^2}\right)^4 + \left(\frac{d^2y}{dx^2}\right)^3 = 0\)
is \ldots\ldots..}

\begin{solutionbox}
(b) 2

\textbf{Solution}: The highest derivative present is
\(\frac{d^2y}{dx^2}\), which is a second derivative. Therefore, the
order is 2.

\end{solutionbox}
\subsubsection{Q1.12 [1 mark]}\label{q1.12-1-mark}

\textbf{Integrating factor of differential equation
\(\frac{dy}{dx} + y = 3x\) is \ldots\ldots..}

\begin{solutionbox}
(c) \(e^x\)

\textbf{Solution}: For the linear differential equation
\(\frac{dy}{dx} + Py = Q\), where \(P = 1\) Integrating factor =
\(e^{\int P dx} = e^{\int 1 dx} = e^x\)

\end{solutionbox}
\subsubsection{Q1.13 [1 mark]}\label{q1.13-1-mark}

\textbf{If given data is 6, 9, 7, 3, 8, 5, 4, 8, 7 and 8 then mean is
\ldots\ldots..}
...
\textbf{Solution}: Mean =
\(\frac{\text{Sum of all values}}{\text{Number of values}}\) Sum =
\(6 + 9 + 7 + 3 + 8 + 5 + 4 + 8 + 7 + 8 = 65\) Number of values = 10
Mean = \(\frac{65}{10} = 6.5\)

\end{solutionbox}
\subsubsection{Q1.14 [1 mark]}\label{q1.14-1-mark}

\textbf{The mean value of first eight natural numbers is \ldots\ldots..}

\begin{solutionbox}
(b) 4.5

\textbf{Solution}: First eight natural numbers: 1, 2, 3, 4, 5, 6, 7, 8
Sum = \(1 + 2 + 3 + 4 + 5 + 6 + 7 + 8 = 36\) Mean =
\(\frac{36}{8} = 4.5\)
...
\subsubsection{Q2.A.1 [3 marks]}\label{q2.a.1-3-marks}

\textbf{If \(M = \begin{bmatrix} 2 & 3 \\ 1 & 0 \end{bmatrix}\),
\(N = \begin{bmatrix} 4 & 1 \\ 2 & -3 \end{bmatrix}\) then prove that
\((M + N)^T = M^T + N^T\)}
...
\(M^T = \begin{bmatrix} 2 & 1 \\ 3 & 0 \end{bmatrix}\),
\(N^T = \begin{bmatrix} 4 & 2 \\ 1 & -3 \end{bmatrix}\)
...
\subsubsection{Q2.A.2 [3 marks]}\label{q2.a.2-3-marks}
...
\subsubsection{Q2.A.3 [3 marks]}\label{q2.a.3-3-marks}

\textbf{Solve differential equation \(\frac{dy}{dx} + x^2 e^{-y} = 0\)}

\textbf{Solution}: \(\frac{dy}{dx} = -x^2 e^{-y}\)
...
\subsubsection{Q2.B.1 [4 marks]}\label{q2.b.1-4-marks}

\textbf{Solve \(-5y + 3x = 1\), \(x + 2y - 4 = 0\) using matrices}

\textbf{Solution}: Rewriting the system: \(3x - 5y = 1\) \(x + 2y = 4\)
...
\subsubsection{Q2.B.2 [4 marks]}\label{q2.b.2-4-marks}

\textbf{If \(A + B = \begin{bmatrix} 1 & -1 \\ 3 & 0 \end{bmatrix}\),
\(A - B = \begin{bmatrix} 3 & 1 \\ 1 & 4 \end{bmatrix}\) then find
\((AB)^{-1}\)}

\textbf{Solution}: Adding the equations:
...
\subsubsection{Q2.B.3 [4 marks]}\label{q2.b.3-4-marks}

\textbf{If
\(B = \begin{bmatrix} -4 & -3 & -3 \\ 1 & 0 & 1 \\ 4 & 4 & 3 \end{bmatrix}\)
then prove that \(\text{adj } B = B\)}

\textbf{Solution}: For a \(3 \times 3\) matrix, we need to find the
cofactor matrix and then transpose it.
...
Therefore, \(\text{adj } B = B\). \textbf{Proved.}

\subsection*{Q.3(A) [6 marks]}\label{q.3a-6-marks}

\textbf{Attempt any two}

\subsubsection{Q3.A.1 [3 marks]}\label{q3.a.1-3-marks}
...
\subsubsection{Q3.A.2 [3 marks]}\label{q3.a.2-3-marks}

\textbf{If \(x = a(t + \sin t)\), \(y = a(1 - \cos t)\) then find
\(\frac{dy}{dx}\)}

\textbf{Solution}: \(\frac{dx}{dt} = a(1 + \cos t)\)

\(\frac{dy}{dt} = a \sin t\)

\(\frac{dy}{dx} = \frac{dy/dt}{dx/dt} = \frac{a \sin t}{a(1 + \cos t)} = \frac{\sin t}{1 + \cos t}\)

Using the identity \(\sin t = 2\sin(t/2)\cos(t/2)\) and
\(1 + \cos t = 2\cos^2(t/2)\):

\(\frac{dy}{dx} = \frac{2\sin(t/2)\cos(t/2)}{2\cos^2(t/2)} = \frac{\sin(t/2)}{\cos(t/2)} = \tan(t/2)\)

\subsubsection{Q3.A.3 [3 marks]}\label{q3.a.3-3-marks}

\textbf{Evaluate \(\int_0^{\pi/2} \sin x \cos x \, dx\)}

\textbf{Solution}: Method 1: Using substitution Let \(u = \sin x\), then
\(du = \cos x \, dx\) When \(x = 0\), \(u = 0\); when \(x = \pi/2\),
\(u = 1\)
...
\subsubsection{Q3.B.1 [4 marks]}\label{q3.b.1-4-marks}

\textbf{If \(y = (\sin x)^{\tan x}\) then find \(\frac{dy}{dx}\)}

\textbf{Solution}: Taking natural logarithm of both sides:
\(\ln y = \tan x \ln(\sin x)\)
...
\subsubsection{Q3.B.2 [4 marks]}\label{q3.b.2-4-marks}

\textbf{Find maximum and minimum value of
\(f(x) = 2x^3 - 3x^2 - 12x + 5\)}
...
At \(x = -1\): \(f''(-1) = -12 - 6 = -18 < 0\) (Maximum) At \(x = 2\):
\(f''(2) = 24 - 6 = 18 > 0\) (Minimum)
...
\subsubsection{Q3.B.3 [4 marks]}\label{q3.b.3-4-marks}

\textbf{The motion of a particle is given by
\(S = t^3 + 6t^2 + 3t + 5\). Find the velocity and acceleration at
\(t = 3\) sec.}

\textbf{Solution}: Position: \(S = t^3 + 6t^2 + 3t + 5\)

Velocity: \(v = \frac{dS}{dt} = 3t^2 + 12t + 3\)

Acceleration: \(a = \frac{dv}{dt} = 6t + 12\)

At \(t = 3\): Velocity: \(v(3) = 3(9) + 12(3) + 3 = 27 + 36 + 3 = 66\)
units/sec

Acceleration: \(a(3) = 6(3) + 12 = 18 + 12 = 30\) units/sec^{2}

\subsection*{Q.4(A) [6 marks]}\label{q.4a-6-marks}

\textbf{Attempt any two}

\subsubsection{Q4.A.1 [3 marks]}\label{q4.a.1-3-marks}

\textbf{Evaluate \(\int x^2 e^x dx\)}

\textbf{Solution}: Using integration by parts twice: Let \(u = x^2\),
\(dv = e^x dx\) Then \(du = 2x dx\), \(v = e^x\)

\(\int x^2 e^x dx = x^2 e^x - \int 2x e^x dx\)

For \(\int 2x e^x dx\): Let \(u_1 = 2x\), \(dv_1 = e^x dx\) Then
\(du_1 = 2 dx\), \(v_1 = e^x\)
...
\subsubsection{Q4.A.2 [3 marks]}\label{q4.a.2-3-marks}

\textbf{Evaluate \(\int \frac{2x + 3}{(x-1)(x+2)} dx\)}
...
Setting \(x = 1\): \(5 = 3A\), so \(A = \frac{5}{3}\) Setting
\(x = -2\): \(-1 = -3B\), so \(B = \frac{1}{3}\)

\(\int \frac{2x + 3}{(x-1)(x+2)} dx = \int \left(\frac{5/3}{x-1} + \frac{1/3}{x+2}\right) dx\)

\(= \frac{5}{3}\ln|x-1| + \frac{1}{3}\ln|x+2| + C\)

\subsubsection{Q4.A.3 [3 marks]}\label{q4.a.3-3-marks}

\textbf{Find mean using the given information}
...
\subsubsection{Q4.B.1 [4 marks]}\label{q4.b.1-4-marks}

\textbf{Evaluate \(\int_{-1}^{1} \frac{x^5 - 6x}{x - 4} dx\)}

\textbf{Solution}: First, let's perform polynomial long division:
...
At \(x = -1\):
\(-\frac{1}{5} + 1 - \frac{16}{3} + 32 - 250 + 1000\ln 5\)

\(= \left(\frac{2}{5} + \frac{32}{3} + 500 + 1000\ln\frac{3}{5}\right)\)

\(= \frac{6 + 160 + 1500}{15} + 1000\ln\frac{3}{5} = \frac{1666}{15} + 1000\ln\frac{3}{5}\)

\subsubsection{Q4.B.2 [4 marks]}\label{q4.b.2-4-marks}

\textbf{Evaluate \(\int \sin 5x \sin 6x \, dx\)}

\textbf{Solution}: Using the product-to-sum formula:
...
\subsubsection{Q4.B.3 [4 marks]}\label{q4.b.3-4-marks}

\textbf{Calculate the standard deviation for the following data: 6, 7,
9, 11, 13, 15, 8, 10}

\textbf{Solution}: Data: 6, 7, 8, 9, 10, 11, 13, 15 (arranged in order)
\(n = 8\)
...
\subsubsection{Q5.A.1 [3 marks]}\label{q5.a.1-3-marks}

\textbf{Find the mean for the following data:}
...
\subsubsection{Q5.A.2 [3 marks]}\label{q5.a.2-3-marks}

\textbf{Calculate the standard deviation for the following data: 5, 9,
8, 12, 6, 10, 6, 8}

\textbf{Solution}: Data: 5, 6, 6, 8, 8, 9, 10, 12 (arranged in order)
\(n = 8\)
...
\subsubsection{Q5.A.3 [3 marks]}\label{q5.a.3-3-marks}

\textbf{Calculate the Mean for the following data: 5, 15, 25, 35, 45,
55, 65, 75, 85, 95, 75}
...
\subsubsection{Q5.B.1 [4 marks]}\label{q5.b.1-4-marks}
...
\(\mu = e^{\int P dx} = e^{\int \frac{1}{x} dx} = e^{\ln x} = x\) (for
\(x > 0\))

Multiplying the equation by \(\mu = x\): \(x\frac{dy}{dx} +
...
Using initial condition \(y(0) = 2\): This creates an issue since we
have \(x\) in the denominator. Let me reconsider the integrating factor
approach.

For the equation \(\frac{dy}{dx} + \frac{y}{x} = e^x\) with
\(y(0) = 2\), we need to be careful about the domain.

The general solution is: \(y = \frac{e^x(x-1) + C}{x}\) for \(x \neq 0\)

Since we need \(y(0) = 2\), we use L'Hôpital's rule or series expansion
near \(x = 0\).

\textbf{Final Answer:} \(y = e^x + \frac{1}{x}\) (subject to domain
restrictions)

\subsubsection{Q5.B.2 [4 marks]}\label{q5.b.2-4-marks}
...
\subsubsection{Q5.B.3 [4 marks]}\label{q5.b.3-4-marks}

\textbf{Solve differential equation \(\frac{dy}{dx} = \sin(x + y)\)}
...
\subsubsection{For Matrix Problems}\label{for-matrix-problems}
...
\subsubsection{For Differentiation}\label{for-differentiation}
...
\subsubsection{For Integration}\label{for-integration}
...
\subsubsection{For Differential
Equations}\label{for-differential-equations}
...
\item
  \textbf{Chain Rule}: Don't forget the derivative of inner function
\item
  \textbf{Integration by Parts}: Choose u and dv carefully using ILATE
  rule
\item
  \textbf{Differential Equations}: Check initial conditions carefully
\item
  \textbf{Statistics}: Don't confuse population and sample standard
  deviation formulas
\end{enumerate}
...
\tightlist
\item
  \textbf{Time Management}: Spend more time on higher mark questions
\item
  \textbf{Show Work}: Always show intermediate steps for partial credit
\item
  \textbf{Check Units}: Ensure your final answers have appropriate units
\item
  \textbf{Verify}: Quick substitution check for differential equations
\item
  \textbf{Neat Presentation}: Write clearly with proper mathematical
  notation
\end{enumerate}
