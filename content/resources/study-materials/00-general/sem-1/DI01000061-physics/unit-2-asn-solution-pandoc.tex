...
\paragraph{\texorpdfstring{\textbf{Electric Potential Difference (\DeltaV or
V):}}{Electric Potential Difference (\DeltaV or V):}}\label{electric-potential-difference-ux3b4v-or-v}
...
(\Phi):}}{Electric Flux (\Phi):}}\label{electric-flux-ux3c6}
...
\item
  \theta = Angle between electric field and normal to the surface
\end{itemize}
...
\tightlist
\item
  When \theta = 0^\circ (perpendicular): \Phi = EA (maximum)
\item
  When \theta = 90^\circ (parallel): \Phi = 0 (minimum)
\end{itemize}
...
\tightlist
\item
  \epsilon_{0} = Permittivity of free space = 8.85 \times 10^{-}^{1}^{2} F/m
\item
  \epsilonᵣ or K = Relative permittivity (dielectric constant)
\item
...
\begin{verbatim}
F = (1/4\pi\epsilon_{0}) \times (q_{1}q_{2})/r^{2}
\end{verbatim}
...
\tightlist
\item
  \epsilon_{0} = Permittivity of free space = 8.85 \times 10^{-}^{1}^{2} C^{2}/N·m^{2}
\item
  k = 1/(4\pi\epsilon_{0}) = 8.9875 \times 10^{9} \approx 9 \times 10^{9} N·m^{2}/C^{2}
\end{itemize}

\textbf{In a Medium:}

\begin{verbatim}
F = (1/4\pi\epsilon_{0}\epsilonᵣ) \times (q_{1}q_{2})/r^{2} = k(q_{1}q_{2})/(\epsilonᵣr^{2})
\end{verbatim}
...
\tightlist
\item
  \epsilonᵣ = Relative permittivity or dielectric constant (K)
\end{itemize}
...
\item
  Permittivity (\epsilon) represents the resistance of the medium that impedes
  the electric field
\end{enumerate}
...
\begin{verbatim}
C = Q/V = \epsilon_{0}A/d
\end{verbatim}

\textbf{With Dielectric:}

\begin{verbatim}
C = \epsilon_{0}\epsilonᵣA/d = \epsilon_{0}KA/d = \epsilonA/d
\end{verbatim}
...
\item
  C = Capacitance (F)
\item
  \epsilon_{0} = 8.85 \times 10^{-}^{1}^{2} F/m
\item
  \epsilonᵣ or K = Dielectric constant
\item
  A = Area of each plate (m^{2})
\item
  d = Distance between plates (m)
\item
  \epsilon = \epsilon_{0}\epsilonᵣ = Permittivity of medium
\end{itemize}
...
For C_{1} = 10 \muF, C_{2} = 20 \muF, C_{3} = 30 \muF in series:
...
For C_{1} = 10 \muF, C_{2} = 20 \muF, C_{3} = 30 \muF in parallel:
...
\begin{verbatim}
Q_{1} = C_{1}V = 10 \times 12 = 120 \muC
Q_{2} = C_{2}V = 20 \times 12 = 240 \muC
Q_{3} = C_{3}V = 30 \times 12 = 360 \muC
Total

Q = 120 + 240 + 360 = 720 \muC
...
For C_{1} = C_{2} = C_{3} = C = 10 \muF:

\textbf{Series:}

\begin{verbatim}
C_{s} = C/n = 10/3 = 3.33 \muF
\end{verbatim}

\textbf{Parallel:}

\begin{verbatim}
C_{p} = nC = 3 \times 10 = 30 \muF
\end{verbatim}
...
\item
  \epsilon_{0} = 8.85 \times 10^{-}^{1}^{2} F/m (permittivity of free space)
\item
  A = Area of plates
\item
  d = Distance between plates
...
\begin{verbatim}
C = \epsilon_{0}\epsilonᵣA/d = \epsilon_{0}KA/d
...
Where:
- \epsilon = \epsilon_{0}\epsilonᵣ = absolute permittivity of medium
- \epsilon_{0} = 8.85 \times 10^{-}^{1}^{2} F/m
- \epsilonᵣ = K = dielectric constant (\geq 1)
\end{verbatim}
...
\tightlist
\item
  Charge 1: q_{1} = 20 \muC = 20 \times 10^{-}^{6} C
\item
  Charge 2: q_{2} = 10 \muC = 10 \times 10^{-}^{6} C
\item
Distance:
...
\begin{solutionbox}
The capacitance of the capacitor is \textbf{5 \muF
...
\begin{solutionbox}
Total capacitance in series = \textbf{3.33 \muF}
or \textbf{10/3 \muF}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Series} & C_{s} = C/n & 3.33 \muF \\
\textbf{Parallel} & C_{p} = nC & 30 \muF \\
\end{longtable}
...
\item
  Permittivity of free space: \epsilon_{0} = 8.85 \times 10^{-}^{1}^{2} F/m
\end{itemize}
...
\begin{verbatim}
C = \epsilon_{0}\epsilonᵣA/d = \epsilon_{0}A/d  (for air, \epsilonᵣ = 1)
\end{verbatim}
...
\item
  Medium: Air (assuming air, \epsilonᵣ = 1)
\item
  Permittivity of free space: \epsilon_{0} = 8.85 \times 10^{-}^{1}^{2} F/m
\end{itemize}
...
\item
  \textbf{Practical capacitors} use microfarads (\muF), nanofarads (nF),
  or picofarads (pF)
\item
...
\begin{verbatim}
A = Cd/(\epsilon_{0}K) = 113 \times 10^{6}/1000 = 113,000 m^{2} \approx 0.113 km^{2}
\end{verbatim}
...
Example: C_{1} = 10 \muF, C_{2} = 20 \muF, C_{3} = 30 \muF
...
C_{t}_{o}_{t}_{a}_{l} = 30/2 = 15 \muF
\end{verbatim}

\begin{solutionbox}
Total capacitance = \textbf{15 \muF}
...
Example: C_{1} = 6 \muF, C_{2} = 6 \muF, C_{3} = 4 \muF
...
C_{t}_{o}_{t}_{a}_{l} = 3 + 4 = 7 \muF
\end{verbatim}

\begin{solutionbox}
Total capacitance = \textbf{7 \muF}
...
\begin{verbatim}
C = Q/V
C (parallel plate) = \epsilon_{0}\epsilonᵣA/d
