...
\item
  θ_{1} (or i) = angle of incidence
\item
  θ_{2} (or r) = angle of refraction
\end{itemize}
...
\begin{verbatim}
sin θc = n_{2} / n_{1}  (where n_{1} > n_{2})
\end{verbatim}
...
\item
  ...
  bends towards the normal (θ_{2} \textless{} θ_{1})
\item
  When light travels from denser to rarer medium (glass to air), it
  bends away from the normal (θ_{2} \textgreater{} θ_{1})
\item
...
\tightlist
\item
  Ray A: i \textless{} θc \rightarrow Refraction occurs (bends away from normal)
\item
Ray B:

i = θc \rightarrow Refracted ray grazes along the surface (r = 90^\circ)

\item
  Ray C: i \textgreater{} θc \rightarrow Total internal reflection occurs
\end{itemize}
...
\item
  ...
  \item
    θc = sin^{-}^{1}(1.00/1.50) = sin^{-}^{1}(0.667) = \textbf{41.1^\circ}
  \end{itemize}
\item
  ...
  \item
    θc = sin^{-}^{1}(1.00/1.33) = sin^{-}^{1}(0.752) = \textbf{48.75^\circ}
  \end{itemize}
\item
  ...
  \item
    θc = sin^{-}^{1}(1.00/2.42) = sin^{-}^{1}(0.413) = \textbf{24.4^\circ}
  \item
...
Figure 4.4 - Normal Bulb Light:
   ...
   ╱   λ_{1}  ╲ (spreads in all directions)
  │  λ_{2} λ_{3} │ 
   ╲  λ_{4}   ╱
    ╲─────╱
...
\tightlist
\item
  \textbf{Core diameter:} Very small (8-10 μm)
\item
  \textbf{Cladding diameter:} 125 μm
\item
...
\tightlist
\item
  \textbf{Core diameter:} Large (50-62.5 μm)
\item
  \textbf{Cladding diameter:} 125 μm
\item
...
\item
  \textbf{Diameter:} 125 μm (standard)
\item
...
\emph{Figure: Light ray entering optical fiber at acceptance angle θ_{a},
...
\emph{Figure: Acceptance cone showing the solid angle (2θ_{a}) within which
...
\item
  The difference in refractive indices (Δn = n_{1} - n_{2}) is small
...
\item
  \textbf{Numerical Aperture (NA):} sin θ_{a} = \sqrt(n_{1}^{2} - n_{2}^{2}) - measures
...
Acceptance Angle: sin θ_{a} = NA / n_{0}
\end{verbatim}
...
\item
  \textbf{Acceptance Angle (θ_{a}) = 26.5^\circ}
\end{itemize}
...
\item
  \textbf{Acceptance Angle (θ_{a}) = 17.3^\circ}
\end{itemize}
...
\begin{verbatim}
NA = \sqrt(n_{1}^{2} - n_{2}^{2}) = n_{0} sin θ_{a}
\end{verbatim}
...
\subsubsection{For small Δ}\label{for-small-ux3b4}

\begin{verbatim}
NA \approx n_{1}\sqrt(2Δ)
\end{verbatim}
