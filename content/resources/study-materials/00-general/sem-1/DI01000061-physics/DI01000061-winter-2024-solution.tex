...
\input{../../../../templates/english-boxes.tex}
...
\textbf{Given:}
\begin{align*}
\text{Capacitance } (C) &= 4~\mu\text{F} = 4 \times 10^{-6}\text{ F}\\
...
\textbf{Answer: Electric charge stored = 48 $\mu$C}
\end{solutionbox}
...
\node[below] at (1,0) {\scriptsize Holes (+)};

% N-type region
\draw[thick,fill=blue!20] (2,0) rectangle (4,1.5);
\node at (3,0.75) {\textbf{N-type}};
\node[below] at (3,0) {\scriptsize Electrons (-)};

% Junction line
\draw[very thick,red] (2,0) -- (2,1.5);
\node[right,rotate=90] at (2,0.75) {\scriptsize Junction};

% Depletion region indicator
\draw[dashed] (1.7,0) -- (1.7,1.5);
\draw[dashed] (2.3,0) -- (2.3,1.5);
\node[above,font=\tiny] at (2,1.6) {Depletion Region};

% Terminals
\draw[thick] (0,0.75) -- (-0.5,0.75) node[left] {Anode};
\draw[thick] (4,0.75) -- (4.5,0.75) node[right] {Cathode};
\end{tikzpicture}
\end{center}
...
\node[blue] at (0.8,0.6) {\scriptsize Forward};
\node[blue] at (-1.2,-0.4) {\scriptsize Reverse};
\node at (-2,-0.6) {\scriptsize Breakdown};
\end{tikzpicture}
\end{center}
