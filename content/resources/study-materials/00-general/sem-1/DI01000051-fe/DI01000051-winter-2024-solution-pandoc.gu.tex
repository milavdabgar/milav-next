...
\textbf{રેઝિસ્ટરની કલર બેન્ડ કોડિંગ પદ્ધતિ સમજાવો. 32 \Omega \pm 10\% કિંમતનો કલર બેન્ડ
લખો.}
...
\textbf{ગણતરી:} 3 \times 2 \times 0.1 = 3.2 \times 10 = 32 \Omega
...
\bottomrule\noalign{}
\endlastfoot
\textbf{કરંટ ગેઇન} & \beta = IC/IB (ઉચ્ચ) \\
...
\textbf{કરંટ ગેઇન \alpha, \beta અને \gamma વચ્ચેનો સંબંધ મેળવો.}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{\alpha (આલ્ફા)} & કોમન બેસ & \alpha = IC/IE \\
\textbf{\beta (બીટા)} & કોમન એમિટર & \beta = IC/IB \\
\textbf{\gamma (ગામા)} & કોમન કલેક્ટર & \gamma = IE/IB \\
\end{longtable}
...
\textbf{પગલું 2: IE ના સંદર્ભમાં IC વ્યક્ત કરો} \alpha = IC/IE તેથી: IC = \alpha \times IE
\ldots{} (1)

\textbf{પગલું 3: કરંટ સમીકરણમાં બદલો} IE = IB + \alpha \times IE IE - \alpha \times IE = IB IE(1
- \alpha) = IB IE = IB/(1 - \alpha) \ldots{} (2)

\textbf{પગલું 4: \beta શોધો} \beta = IC/IB (1) થી: IC = \alpha \times IE (2) થી: IE = IB/(1
- \alpha) તેથી: IC = \alpha \times IB/(1 - \alpha)

\textbf{પગલું 5: \beta માટે અંતિમ સંબંધ} \beta = IC/IB = \alpha/(1 - \alpha) \ldots{} (3)

\textbf{પગલું 6: \beta ના સંદર્ભમાં \alpha વ્યક્ત કરો} સમીકરણ (3) થી: \beta(1 - \alpha) = \alpha \beta -
\beta\alpha = \alpha \beta = \alpha + \beta\alpha = \alpha(1 + \beta) તેથી: \alpha = \beta/(1 + \beta) \ldots{} (4)

\textbf{પગલું 7: \gamma શોધો} \gamma = IE/IB (2) થી: \gamma = 1/(1 - \alpha) (4) થી \alpha બદલતાં: \gamma
= 1/(1 - \beta/(1 + \beta)) \gamma = (1 + \beta)/(1 + \beta - \beta) \gamma = 1 + \beta \ldots{} (5)
...
\bottomrule\noalign{}
\endlastfoot
\textbf{\alpha ના સંદર્ભમાં \beta} & \beta = \alpha/(1 - \alpha) \\
\textbf{\beta ના સંદર્ભમાં \alpha} & \alpha = \beta/(1 + \beta) \\
\textbf{\beta ના સંદર્ભમાં \gamma} & \gamma = 1 + \beta \\
\textbf{ચકાસણી} & \alpha + \beta \times \alpha = \beta \\
\end{longtable}
...
\tightlist
\item
  \alpha \approx 0.98 થી 0.995
\item
  \beta \approx 50 થી 200\\
\item
  \gamma \approx 51 થી 201
\end{itemize}
...
\item
  \textbf{IC = \beta \times IB} સંબંધ લાગુ
\item
...
\tightlist
\item
  \textbf{કરંટ ગેઇન}: \beta (50-200 સામાન્ય)
\item
...
\textbf{કરંટ ગેઇન} & \alpha \textless{} 1 & \beta \textgreater\textgreater{} 1 & \gamma
= (1 + \beta) \\
\textbf{વોલ્ટેજ ગેઇન} & ઉચ્ચ & ઉચ્ચ & \textless{} 1 (\approx1) \\
\textbf{પાવર ગેઇન} & મધ્યમ & ખૂબ ઉચ્ચ & મધ્યમ \\
\textbf{ઇનપુટ રેઝિસ્ટન્સ} & ખૂબ ઓછું (20-50\Omega) & મધ્યમ (1-5k\Omega) & ખૂબ ઉચ્ચ
(100k\Omega) \\
\textbf{આઉટપુટ રેઝિસ્ટન્સ} & ખૂબ ઉચ્ચ (1M\Omega) & ઉચ્ચ (50k\Omega) & ઓછું (25\Omega) \\
...
\textbf{ડિઝાઇન ઉદાહરણ:} T = 1ms માટે: જો C = 0.1µF, તો R = 9.1k\Omega
...
\item
  C = 0.1µF પસંદ કરો
\item
  R1 = 7.2k\Omega, R2 = 3.6k\Omega ગણતરી કરો
\end{itemize}
