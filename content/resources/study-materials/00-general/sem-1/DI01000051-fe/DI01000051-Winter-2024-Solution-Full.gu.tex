...
\begin{tikzpicture}
    ...
    % Leads
    \draw[thick] (-0.8,0) -- (-0.8,1) node[above] {Positive (+)};
    \draw[thick] (0.8,0) -- (0.8,1) node[above] {Negative (-)};
\end{tikzpicture}
...
\begin{circuitikz}[scale=1]
    ...
    \draw (T.A1) -- ++(-2,0) to[sV, l=AC Input] ++(0,-2) -- (T.A2);
    
    % Secondary Side
    \draw (T.B1) to[diode, l=D1] ++(2,0) coordinate (D1_cat);
    \draw (T.B2) to[diode, l=D2] ++(2,0) coordinate (D2_cat);
    
    % Join Diodes (Positive Rail)
    \draw (D1_cat) -- (D2_cat);
    \draw ($(D1_cat)!0.5!(D2_cat)$) coordinate (mid) -- ++(2.5,0) coordinate (out_top);
    
    % Center Tap (Negative Rail/Ground)
    % Start from actual center of secondary winding, go right slightly then down crossing the lower wire
    \draw ($(T.B1)!0.5!(T.B2)$) -- ++(0.1,0) -- ++(0,-2.0) coordinate (bot_turn);
    \draw (bot_turn) -- (bot_turn -| out_top) coordinate (out_bot);
    
    % Load Resistor (Vertical)
    \draw (out_top) to[R, l=\(R_L\)] (out_bot);
\end{circuitikz}
...
\begin{tikzpicture}[scale=1]
    % NPN Block (Widened to 3.5 per block)
    \draw[thick] (0,0) rectangle (3.5,2) node[midway] {N (Emitter)};
    \draw[thick] (3.5,0) rectangle (4.5,2) node[midway] {P};
    \draw[thick] (4.5,0) rectangle (8,2) node[midway] {N (Collector)};
    
    % Biasing (Center at x=4)
    \draw (0,1) -- (-1,1) -- (-1,-1) to[battery1, l=\(V_{BE}\)] (4,-1) -- (4,0);
    \draw (8,1) -- (9,1) -- (9,-1) to[battery1, l=\(V_{CB}\)] (4,-1);
    
    % Electron Flow
    \draw[->, dashed] (0.5, 0.5) -- (7.5, 0.5) node[right] {Electrons};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[scale=1]
    \draw[thick] (0,0) rectangle (3,2) node[midway] {P (Emitter)};
    \draw[thick] (3,0) rectangle (6,2) node[midway] {N (Base)};
    \draw[thick] (6,0) rectangle (9,2) node[midway] {P (Collector)};
    \draw (0,1) -- (-0.5,1) node[left] {E};
    \draw (4.5,2) -- (4.5,2.5) node[above] {B};
    \draw (9,1) -- (9.5,1) node[right] {C};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.8]
    % NPN
    \draw (0,0) rectangle (2,2) node[midway] {N};
    \draw (2,0) rectangle (4,2) node[midway] {P};
    \draw (4,0) rectangle (6,2) node[midway] {N};
    \node at (3,-0.5) {NPN Construction};
    
    % PNP
    \draw (7,0) rectangle (9,2) node[midway] {P};
    \draw (9,0) rectangle (11,2) node[midway] {N};
    \draw (11,0) rectangle (13,2) node[midway] {P};
    \node at (10,-0.5) {PNP Construction};
\end{tikzpicture}
...
\begin{circuitikz}[scale=1]
    ...
    \draw (T.B) to[short] (-2,0) coordinate (B_node);
    \draw (B_node) to[C, l=\(C_{in}\)] (-4.5,0) to[sV, l=\(V_{in}\)] (-4.5,-2) node[ground]{};
    \draw (0,2) to[R, l=\(R_B\)] (-2, 2) -- (B_node); % Collector Feedback Bias (connecting Collector to Base)
...
\begin{circuitikz}[scale=1]
    ...
    \node at (3,2.5) {\Large \textbf{IC 555}};
    
    % Left: Pin 2 (Trig)
    \draw (0,2.5) -- (-1.5,2.5) to[sV, l=Trig] (-1.5,0.5) node[ground]{};
    \node at (0.2, 2.5) [right, font=\tiny] {2 (Trig)};
    
    % Right: Pin 3 (Out)
    \draw (6,2.5) -- (7.5,2.5) node[right]{Output};
    \node at (5.8, 2.5) [left, font=\tiny] {3 (Out)};
    
    % Top: Pin 8 (Vcc), 4 (Reset)
    \draw (2,5) -- (2,6) node[vcc]{Vcc}; \node at (2, 4.8) [below, font=\tiny] {8};
    \draw (4,5) -- (4,6) -- (2,6); \node at (4, 4.8) [below, font=\tiny] {4};
    
    % Top Right: Pin 7 (Disch), 6 (Thres)
    % RA from Vcc to 7
    \draw (2,6) -| (7,6) to[R, l=\(R_A\)] (7, 4.5) coordinate (p7);
    \draw (6, 4.5) -- (p7); \node at (5.8, 4.5) [left, font=\tiny] {7};
    \draw (6, 3.5) -- (7, 3.5) coordinate (p6); \node at (5.8, 3.5) [left, font=\tiny] {6};
    
    % Wiring 6 and 7 together (Monostable: 6&7 connected)
    \draw (p7) -- (p6);
    \draw (p6) to[C, l=\(C\)] (7, 0) node[ground]{};
    
    % Bottom: Pin 1 (GND), 5 (CV)
    \draw (2,0) -- (2,-1) node[ground]{} node[right, pos=0.5]{1 (GND)}; \node at (2,0.2) [above, font=\tiny] {1};
    \draw (4,0) to[C, l=0.01\(\mu\)F] (4,-1.5) node[ground]{}; \node at (4,0.2) [above, font=\tiny] {5 (CV)};
\end{circuitikz}
...
\begin{circuitikz}[scale=1]
    ...
    % Left: No inputs for Astable (Self Trigger)
    % We can visually balance by putting nothing or just Grounding
    
    % Right: Pin 3 (Out)
    \draw (6,2.5) -- (7.5,2.5) node[right]{Output (Square Wave)};
    \node at (5.8, 2.5) [left, font=\tiny] {3 (Out)};
    ...
    % Top Right Chain: Vcc -> RA -> 7 -> RB -> 6/2 -> C -> GND
    \draw (2,6) -| (7,6) to[R, l=\(R_A\)] (7, 4.5) coordinate (p7);
    \draw (6, 4.5) -- (p7); \node at (5.8, 4.5) [left, font=\tiny] {7};
    
    \draw (p7) to[R, l=\(R_B\)] (7, 2) coordinate (p62);
    \draw (6, 3.5) -- (6.5, 3.5) -- (6.5, 2) -- (p62); \node at (5.8, 3.5) [left, font=\tiny] {6}; % Pin 6
    \draw (6, 1.5) -- (6.5, 1.5) -- (6.5, 2); \node at (5.8, 1.5) [left, font=\tiny] {2}; % Pin 2
    
    \draw (p62) to[C, l=\(C\)] (7, -1) node[ground]{};
    
    % Bottom: Pin 1 (GND), 5 (CV)
    \draw (2,0) -- (2,-1) node[ground]{1 (GND)}; \node at (2,0.2) [above, font=\tiny] {1};
    \draw (4,0) to[C, l=0.01\(\mu\)F] (4,-1.5) node[ground]{}; \node at (4,0.2) [above, font=\tiny] {5 (CV)};
\end{circuitikz}
