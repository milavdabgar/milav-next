...
band of 32 \Omega \pm10\% resistance.}
...
\textbf{Calculation:} 3 \times 2 \times 0.1 = 3.2 \times 10 = 32 \Omega
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Current Gain} & \beta = IC/IB (high) \\
...
\bottomrule\noalign{}
\endlastfoot
\textbf{\alpha (Alpha)} & Common Base & \alpha = IC/IE \\
\textbf{\beta (Beta)} & Common Emitter & \beta = IC/IB \\
\textbf{\gamma (Gamma)} & Common Collector & \gamma = IE/IB \\
\end{longtable}
...
\textbf{Step 2: Express IC in terms of IE} \alpha = IC/IE Therefore: IC = \alpha \times
IE \ldots{} (1)

\textbf{Step 3: Substitute in current equation} IE = IB + \alpha \times IE IE - \alpha
\times IE = IB IE(1 - \alpha) = IB IE = IB/(1 - \alpha) \ldots{} (2)

\textbf{Step 4: Find \beta} \beta = IC/IB From (1): IC = \alpha \times IE From (2): IE =
IB/(1 - \alpha) Therefore: IC = \alpha \times IB/(1 - \alpha)

\textbf{Step 5: Final relation for \beta} \beta = IC/IB = \alpha/(1 - \alpha) \ldots{} (3)

\textbf{Step 6: Express \alpha in terms of \beta} From equation (3): \beta(1 - \alpha) = \alpha
\beta - \beta\alpha = \alpha \beta = \alpha + \beta\alpha = \alpha(1 + \beta) Therefore: \alpha = \beta/(1 + \beta) \ldots{} (4)

\textbf{Step 7: Find \gamma} \gamma = IE/IB From (2): \gamma = 1/(1 - \alpha) Substituting \alpha
from (4): \gamma = 1/(1 - \beta/(1 + \beta)) \gamma = (1 + \beta)/(1 + \beta - \beta) \gamma = 1 + \beta
\ldots{} (5)
...
\bottomrule\noalign{}
\endlastfoot
\textbf{\beta in terms of \alpha} & \beta = \alpha/(1 - \alpha) \\
\textbf{\alpha in terms of \beta} & \alpha = \beta/(1 + \beta) \\
\textbf{\gamma in terms of \beta} & \gamma = 1 + \beta \\
\textbf{Verification} & \alpha + \beta \times \alpha = \beta \\
\end{longtable}
...
\tightlist
\item
  \alpha \approx 0.98 to 0.995
\item
  \beta \approx 50 to 200\\
\item
  \gamma \approx 51 to 201
\end{itemize}
...
\item
  \textbf{IC = \beta \times IB} relationship holds
\item
...
\tightlist
\item
  \textbf{Current gain}: \beta (50-200 typical)
\item
...
\textbf{Current Gain} & \alpha \textless{} 1 & \beta \textgreater\textgreater{} 1
& \gamma = (1 + \beta) \\
\textbf{Voltage Gain} & High & High & \textless{} 1 (\approx1) \\
\textbf{Power Gain} & Medium & Very High & Medium \\
\textbf{Input Resistance} & Very Low (20-50\Omega) & Medium (1-5k\Omega) & Very
High (100k\Omega) \\
\textbf{Output Resistance} & Very High (1M\Omega) & High (50k\Omega) & Low
(25\Omega) \\
...
\textbf{Design Example:} For T = 1ms: If C = 0.1µF, then R = 9.1k\Omega
...
\item
  Choose C = 0.1µF
\item
  Calculate R1 = 7.2k\Omega, R2 = 3.6k\Omega
\end{itemize}
