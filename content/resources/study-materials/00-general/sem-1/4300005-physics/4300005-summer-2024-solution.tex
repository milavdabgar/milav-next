...
\textbf{The length of a metal rod is 64.522 cm at 12°C temperature and
64.576 cm at 90°C temperature. Find the coefficient of linear expansion
of the metal rod.}

\begin{solutionbox}
\textbf{Formula:} α = (L₂ - L₁)/{[}L₁ × (T₂ - T₁){]}
...
\item
  \textbf{Reading}: Main scale reading + (Circular scale reading × Least
  count)
\item
...
\end{solutionbox}
\begin{mnemonicbox}
``MSR + (CSR × LC) gives the final measurement''
...
\textbf{Parallel Plate Capacitor:} Capacitance formula: C = ε₀A/d
...
\textbf{If the lengths of two cylinders are (6.52±0.01) cm and
(4.48±0.02) cm respectively. Find the difference in their length with
percentage error.}
...
\textbf{Absolute error in difference} = √{[}(0.01)² + (0.02)²{]} =
√(0.0001 + 0.0004) = √0.0005 = 0.022 cm

\textbf{Percentage error} = (Absolute error/Measured value) × 100 =
(0.022/2.04) × 100 = 1.08\%
...
\bottomrule\noalign{}
\endlastfoot
Constructive & 0°, 360°, 720°\ldots{} & Reinforcement & Maximum \\
Destructive & 180°, 540°, 900°\ldots{} & Cancellation & Minimum \\
\end{longtable}
}
...
\textbf{Final formula:} V = (1/4πε₀) × (Q/r)
...
\tightlist
\item
  Distance (d) = n × λ = 50 × 1.328 = 66.4 m
\end{enumerate}
...
\end{solutionbox}
\begin{mnemonicbox}
``PEW: Potential difference × Electric charge =
Work''
...
separated by distance 5×10⁻¹⁰ m from eachother is 3.7 × 10⁻⁹ N. How many
electrons would have been removed from each atom.}

\begin{solutionbox}
\textbf{Formula:} F = (1/4πε₀) × (q₁q₂/r²)
...
(n² × (1.6 × 10⁻¹⁹)²/25 × 10⁻²⁰) Solving: n = 1 (1 electron removed from
each atom)
...
\textbf{Formula:} (sin i)/(sin r) = n₂/n₁ = constant
...
\begin{solutionbox}
\textbf{Parallel Combination:} Cₚ = C₁ + C₂ + C₃ = 5 +
10 + 15 = 30 µF

\textbf{Series Combination:} 1/Cₛ = 1/C₁ + 1/C₂ + 1/C₃ 1/Cₛ = 1/5 + 1/10
+ 1/15 1/Cₛ = 0.2 + 0.1 + 0.067 = 0.367 Cₛ = 1/0.367 = 2.72 µF
...
raise the temperature of 1 kg of a substance by 1 Kelvin (or 1°C).
...
Critical angle (θc) = sin⁻¹(n₂/n₁) θc = sin⁻¹(1.45/1.48) θc =
sin⁻¹(0.9797) θc = 78.4°
