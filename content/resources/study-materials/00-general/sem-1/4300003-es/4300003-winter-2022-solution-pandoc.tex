...
\item
  Standard test: 5 days at 20^\circC
\end{itemize}
...
\textbf{Power calculation}: P = 0.5 \times ρ \times A \times V^{3} \times Cp Where: ρ = air
density,

A = swept area,

V = wind speed, Cp = power coefficient
...
\textbf{Optimal conditions}: Temperature 35-40^\circC, pH 6.8-7.2, retention
time 15-30 days.
...
\tightlist
\item
  \textbf{Global temperature rise}: Average 1.1^\circC increase since
...
\bottomrule\noalign{}
\endlastfoot
Temperature & +1.1^\circC since 1880 & Global temperature records \\
...
\tightlist
\item
  \textbf{Temperature goal}: Limit global warming to well below 2^\circC,
  preferably 1.5^\circC
\item
...
\item
  \textbf{Calculation}: Collection = Catchment area \times Rainfall \times Runoff
  coefficient
\end{itemize}
...
\item
  Harvestable water = 100 \times 1 \times 0.8 = 80,000 liters/year
\end{itemize}
