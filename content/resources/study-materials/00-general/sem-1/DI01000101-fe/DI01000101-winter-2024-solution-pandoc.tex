...
\end{solutionbox}
\begin{mnemonicbox}
``Flux Change Generates EMF'' (dΦ/dt = EMF)
...
\bottomrule\noalign{}
\endlastfoot
\textbf{KVL} & Sum of voltages in closed loop = 0 & ΣV = 0 & Series
circuits \\
\textbf{KCL} & Sum of currents at node = 0 & ΣI = 0 & Parallel
circuits \\
\end{longtable}
...
\item
  \textbf{Φ_{m}}: Maximum flux, \textbf{f}: Frequency
\end{itemize}
...
\tightlist
\item
  \textbf{Armature control}: N ∝ (V - IaRa)/φ
\item
  \textbf{Field control}: N ∝ V/φ
\item
  \textbf{Resistance control}: N ∝ (V - Ia(Ra + Rext))/φ
\end{itemize}
...
\textbf{Angular velocity} & ω & rad/s & Rotation speed \\
\textbf{Phase angle} & φ & Degrees & Initial phase \\
\textbf{Frequency} & f = ω/2π & Hz & Cycles per second \\
\end{longtable}
...
Amperes \\
\textbf{Average Value} & Mean value over half cycle & 2Im/π & Amperes \\
...
\tightlist
\item
  \textbf{Form Factor}: RMS/Average = π/2\sqrt2 = 1.11
\item
  \textbf{Peak Factor}: Peak/RMS = \sqrt2 = 1.414
\item
  \textbf{Angular frequency}: ω = 2πf
\end{itemize}
...
\textbf{Power} & P = \sqrt3 \times VL \times IL \times cosφ \\
\end{longtable}
...
\textbf{Phase} & φ & Leading/lagging angle \\
\textbf{Angular velocity} & ω & Rotation speed \\
...
wave) \\
\textbf{Angular Velocity} & Rate of phase change & ω = 2πf & 314 rad/s
(50Hz) \\
...
Ohms \\
\textbf{Phase Angle} & Angle between V and I & φ = tan^{-}^{1}(X/R) &
Degrees \\
\textbf{Power Factor} & Cosine of phase angle & PF = cosφ = R/Z & - \\
\textbf{Reactive Power} & Power in reactive components & Q = VI sinφ &
VAR \\
\end{longtable}
...
\tightlist
\item
  \textbf{Active Power}: P = VI cosφ (Watts)
\item
  \textbf{Reactive Power}: Q = VI sinφ (VAR)
\item
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Current amplitude} & Im = Vm/ωL & Current lags voltage by 90^\circ \\
\textbf{Inductive reactance} & XL = ωL = 2πfL & Frequency dependent \\
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Active Power} & P & VI cosφ & Watts & Useful power \\
\textbf{Reactive Power} & Q & VI sinφ & VAR & Circulating power \\
