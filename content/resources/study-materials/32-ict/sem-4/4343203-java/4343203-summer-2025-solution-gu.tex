...
\title{Java Programming (4343203) -- સમર 2025 સોલ્યુશન}
\date{May 13, 2025}
...
\begin{tikzpicture}[node distance=1.5cm]
    ...
    \node (dead) [process, right of=running, xshift=1.5cm] {Dead};

    \draw [arrow] (new) -- (runnable);
    \draw [arrow] (runnable) -- (running);
    \draw [arrow] (running) -- (blocked);
    \draw [arrow] (blocked) -| (runnable);
    \draw [arrow] (running) -- (dead);
\end{tikzpicture}
...
\hline
\textbf{Access Specifier} & \textbf{Class} & \textbf{Package} & \textbf{Subclass} & \textbf{Other} \\ \hline
