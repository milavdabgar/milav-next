...
\begin{tikzpicture}[node distance=1.5cm, auto]
    \node (code) [process] {Java Code};
    \node (comp) [process, below of=code] {Compiler};
    \node (byte) [process, below of=comp] {Bytecode};
    \node (jvm) [process, below of=byte] {JVM};
    \node (mach) [process, below of=jvm] {Machine Code};

    \draw [arrow] (code) -- (comp);
    \draw [arrow] (comp) -- (byte);
    \draw [arrow] (byte) -- (jvm);
    \draw [arrow] (jvm) -- (mach);
    
    \node (comp_box) [draw=red, dashed, fit=(jvm), label=right:JVM ઘટકો: Class Loader, Runtime Data, Exec Engine] {};
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=1.5cm]
    \node (cons) [process] {Constructors};
    \node (def) [process, below left of=cons, xshift=-2cm] {Default};
    \node (param) [process, below of=cons] {Parameterized};
    \node (copy) [process, below right of=cons, xshift=2cm] {Copy};

    \draw [arrow] (cons) -- (def);
    \draw [arrow] (cons) -- (param);
    \draw [arrow] (cons) -- (copy);
\end{tikzpicture}
...
\begin{center}
\begin{tikzpicture}[node distance=1.5cm]
    \node (prt) [class] {$\ll$interface$\gg$ Printable};
    \node (scn) [class, right of=prt, xshift=3cm] {$\ll$interface$\gg$ Scannable};
    \node (impl) [class, below of=prt, xshift=1.5cm] {Printer};

    \draw [dashed, ->] (impl) -- (prt);
    \draw [dashed, ->] (impl) -- (scn);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=1.5cm]
    \node (new) [process] {New};
    \node (runnable) [process, right of=new, xshift=1.5cm] {Runnable};
    \node (running) [process, right of=runnable, xshift=1.5cm] {Running};
    \node (blocked) [process, below of=running] {Blocked};
    \node (term) [process, right of=running, xshift=1.5cm] {Terminated};

    \draw [arrow] (new) -- node[above, font=\tiny] {start()} (runnable);
    \draw [arrow] (runnable) -- node[above, font=\tiny] {scheduler} (running);
    \draw [arrow] (running) -- node[right, font=\tiny] {sleep/wait} (blocked);
    \draw [arrow] (blocked) -| node[above, font=\tiny, xshift=-1cm] {notify} (runnable);
    \draw [arrow] (running) -- node[above, font=\tiny] {complete} (term);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=1cm]
    \node (throw) [class] {Throwable};
    \node (exc) [class, below left of=throw, xshift=-1cm] {Exception};
    \node (err) [class, below right of=throw, xshift=1cm] {Error};
    \node (run) [class, below of=exc] {RuntimeException};
    \node (arith) [class, below left of=run, xshift=-0.5cm] {ArithmeticException};
    \node (null) [class, below right of=run, xshift=0.5cm] {NullPointerException};

    \draw [arrow] (throw) -- (exc);
    \draw [arrow] (throw) -- (err);
    \draw [arrow] (exc) -- (run);
    \draw [arrow] (run) -- (arith);
    \draw [arrow] (run) -- (null);
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[node distance=1cm]
    \node (stream) [process] {Stream};
    
    \node (input) [process, below left of=stream, xshift=-1cm] {Input};
    \node (output) [process, below right of=stream, xshift=1cm] {Output};
    
    \node (byte1) [process, below of=input] {Byte};
    \node (char1) [process, below of=output] {Char};

    \draw [arrow] (stream) -- (input);
    \draw [arrow] (stream) -- (output);
    \draw [arrow] (input) -- (byte1);
    \draw [arrow] (output) -- (char1);
    \draw [arrow] (input) -- (char1); % Just linking roughly to show structure
\end{tikzpicture}
