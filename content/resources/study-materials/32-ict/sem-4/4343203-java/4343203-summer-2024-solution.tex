...
\begin{itemize}
    \item \textbf{Automatic}: No manual memory management required
    \item \textbf{Background}: Runs in separate low-priority thread
\end{itemize}
\end{solutionbox}
...
\begin{tikzpicture}[node distance=1.5cm, auto,
    block/.style={rectangle, draw, fill=blue!10, align=center, rounded corners, minimum height=2em, font=\small},
    line/.style={draw, -latex}]

    \node [block] (java) {Java Code};
    \node [block, right=0.8cm of java] (compiler) {Compiler};
    \node [block, right=0.8cm of compiler] (bytecode) {Bytecode};
    \node [block, right=0.8cm of bytecode] (jvm) {JVM};
    \node [block, right=0.8cm of jvm] (machine) {Machine Code};

    \path [line] (java) -- (compiler);
    \path [line] (compiler) -- (bytecode);
    \path [line] (bytecode) -- (jvm);
    \path [line] (jvm) -- (machine);
    ...
    \path [line, dashed] (jvm) -- (components);
...
\begin{itemize}
    ...
    \item \textbf{Security}: Bytecode verification prevents dangerous operations
    \item \textbf{Optimization}: Just-in-time compilation improves performance
\end{itemize}
\end{solutionbox}
...
\begin{itemize}
    \item \textbf{Initialize}: Start with 0 and 1
    \item \textbf{Loop}: Iterate N times to generate sequence
    \item \textbf{Calculation}: Each number is sum of previous two
\end{itemize}
\end{solutionbox}
...
\textbf{Write a program in java to find out minimum from any ten numbers using command line argument.}
...
\begin{itemize}
    \item \textbf{Parse Arguments}: Convert string arguments to integers
    \item \textbf{Initialize}: Set first number as minimum
    \item \textbf{Compare}: Check each number against current minimum
\end{itemize}
\end{solutionbox}
...
\textbf{List out basic concepts of Java OOP. Explain any one in details.}
...
\begin{itemize}
    \item \textbf{Encapsulation}: Protects data through access control
    \item \textbf{Data Hiding}: Private variables accessible through methods
\end{itemize}
\end{solutionbox}
...
final variable & Cannot be changed & \texttt{final int MAX = 100;} \\ \hline
final method & Cannot be overridden & \texttt{final void display() \{\}} \\ \hline
final class & Cannot be extended & \texttt{final class Math \{\}} \\ \hline
\end{tabular}
\end{center}
...
\textbf{What is constructor? Explain parameterized constructor with example.}
...
\begin{tikzpicture}[node distance=1.5cm, auto,
    block/.style={rectangle, draw, fill=green!10, align=center, rounded corners, minimum height=2em, font=\small},
    line/.style={draw, -latex}]

    \node [block] (root) {Constructors};
    \node [block, below left=1cm of root] (default) {Default Constructor};
    \node [block, below=1cm of root] (param) {Parameterized Constructor};
    \node [block, below right=1cm of root] (copy) {Copy Constructor};

    \path [line] (root) -- (default);
    \path [line] (root) -- (param);
    \path [line] (root) -- (copy);
\end{tikzpicture}
\end{center}
...
\begin{itemize}
    \item \textbf{Parameters}: Accept values during object creation
    \item \textbf{Initialization}: Set object properties with passed values
    \item \textbf{Overloading}: Multiple constructors with different parameters
\end{itemize}
\end{solutionbox}
...
\begin{tikzpicture}[
    ...
    container/.style={draw, rectangle, minimum width=4.5cm, fill=blue!5}
]
    \node[container, minimum height=4cm] (outer) {};
...
\begin{itemize}
    \item \textbf{Package}: Groups related classes
    \item \textbf{Import}: Includes external classes
    \item \textbf{Class}: Contains variables and methods
\end{itemize}
\end{solutionbox}
...
\textbf{Define Inheritance. List out types of it. Explain multilevel and hierarchical inheritance with suitable example.}
...
Multilevel & Chain of inheritance (\texttt{A}$\rightarrow$\texttt{B}$\rightarrow$\texttt{C}) \\ \hline
...
\begin{tikzpicture}[node distance=1.2cm, auto,
    cls/.style={rectangle, draw, fill=yellow!10, align=center, minimum width=2cm},
    line/.style={draw, -latex}]

    % Multilevel
    \node [cls] (A) {Animal};
    \node [cls, below=0.8cm of A] (B) {Dog};
    \node [cls, below=0.8cm of B] (C) {Labrador};
    ...
    % Hierarchical
    \node [cls, right=3cm of A] (V) {Vehicle};
    \node [cls, below left=1cm and -0.5cm of V] (C1) {Car};
    \node [cls, below=1cm of V] (C2) {Bike};
    \node [cls, below right=1cm and -0.5cm of V] (C3) {Truck};
...
\begin{itemize}
    \item \textbf{Private}: Only within the same class
    \item \textbf{Default}: Within the same package
    \item \textbf{Protected}: Within package and subclasses
    \item \textbf{Public}: Accessible everywhere
\end{itemize}
\end{solutionbox}
...
\textbf{What is interface? Explain multiple inheritance using interface with example.}
...
\begin{tikzpicture}[node distance=1.5cm, auto,
    cls/.style={rectangle, draw, fill=orange!10, align=center, minimum width=2cm},
    iface/.style={rectangle, draw, fill=green!10, align=center, minimum width=2cm, dashed},
    line/.style={draw, -latex, dashed}]

    \node [iface] (P) {\textit{<<interface>>}\\Printable};
    \node [iface, right=2cm of P] (S) {\textit{<<interface>>}\\Scannable};
    \node [cls, below left=1.5cm and -1cm of S] (I) {Printer};
...
\begin{itemize}
    \item \textbf{Contract}: Defines behavior without implementation
    \item \textbf{Implements}: Classes fulfill the contract
    \item \textbf{Multiple}: Can implement many interfaces
\end{itemize}
\end{solutionbox}
...
\textbf{What is package? Write steps to create a package and give example of it.}
...
\begin{lstlisting}[language=Java,caption={Multiple Inheritance Using Interfaces}]
...
\textbf{Define: Method Overriding. List out Rules for method overriding. Write a java program that implements method overriding.}
...
\begin{itemize}
    \item \textbf{Runtime Polymorphism}: Method resolution at runtime
    \item \textbf{@Override}: Annotation ensures method is overriding
    \item \textbf{Inheritance}: Requires IS-A relationship
\end{itemize}
\end{solutionbox}
...
\begin{tikzpicture}[node distance=1.5cm, auto,
    state/.style={draw, circle, fill=blue!10, align=center, minimum width=2.5em},
    line/.style={draw, -latex}]

    \node [state] (N) {New};
    \node [state, right=1.5cm of N] (R) {Runnable};
    \node [state, right=1.5cm of R] (Ru) {Running};
    \node [state, right=1.5cm of Ru] (T) {Terminated};
    \node [state, below=1.5cm of Ru] (B) {Blocked};

    \path [line] (N) -- (R);
    \path [line] (R) -- node[font=\tiny]{Scheduler} (Ru);
    \path [line] (Ru) -- (T);
    \path [line] (Ru) -- node[right, font=\tiny]{Wait} (B);
    \path [line] (B) -| node[pos=0.2, above, font=\tiny]{Notify} (R);
\end{tikzpicture}
\end{center}

\begin{itemize}
    \item \textbf{New}: Thread created but not started
    \item \textbf{Runnable}: Ready to run when CPU time is given
    \item \textbf{Running}: Currently executing
    \item \textbf{Blocked/Waiting}: Temporarily inactive
    \item \textbf{Terminated}: Completed execution
\end{itemize}
\end{solutionbox}
...
\textbf{Write a program in java that creates the multiple threads by implementing the Thread class.}
...
\begin{itemize}
    \item \textbf{Extend Thread}: Create thread by extending Thread class
    \item \textbf{Override run()}: Define task in run method
...
\begin{itemize}
    \item \textbf{Security}: Protects sensitive implementations
    \item \textbf{Immutability}: Helps create immutable classes
    \item \textbf{Optimization}: JVM can optimize final classes
\end{itemize}
\end{solutionbox}
...
\textbf{What is Exception? Write a program that shows the use of Arithmetic Exception.}
...
\begin{tikzpicture}[node distance=1cm, auto,
    cls/.style={rectangle, draw, fill=red!10, align=center, minimum width=2.5cm},
    line/.style={draw, -latex}]

    \node [cls] (T) {Throwable};
    \node [cls, below left=1cm of T] (E) {Exception};
    \node [cls, below right=1cm of T] (Err) {Error};
    \node [cls, below=1cm of E] (RE) {RuntimeException};
    \node [cls, below left=1cm and -1.5cm of RE] (AE) {ArithmeticException};
    \node [cls, below right=1cm and -1.5cm of RE] (NPE) {NullPointerException};
...
\begin{itemize}
    \item \textbf{Try Block}: Contains code that might throw exceptions
    \item \textbf{Catch Block}: Handles the specific exception
    \item \textbf{Finally Block}: Always executes regardless of exception
\end{itemize}
\end{solutionbox}
...
\textbf{Write a Java Program to find sum and average of 10 numbers of an array.}
...
\begin{itemize}
    \item \textbf{Declaration}: Creates fixed-size collection
    \item \textbf{Iteration}: Sequential access to elements
    \item \textbf{Calculation}: Process values for results
\end{itemize}
\end{solutionbox}
...
\textbf{Write a Java program to handle user defined exception for `Divide by Zero' error.}
...
\begin{itemize}
    \item \textbf{Custom Class}: Extends Exception class
    \item \textbf{Throwing}: Use throw keyword with new instance
    \item \textbf{Handling}: Catch specific exception type
\end{itemize}
\end{solutionbox}
...
\textbf{Write a java program to create a text file and perform read operation on the text file.}
...
\begin{itemize}
    \item \textbf{FileWriter}: Creates and writes to files
    \item \textbf{FileReader}: Reads character data from files
    \item \textbf{BufferedReader}: Efficiently reads text by lines
\end{itemize}
\end{solutionbox}
...
\begin{tikzpicture}[node distance=1cm, auto,
    cls/.style={rectangle, draw, fill=cyan!10, align=center, minimum width=2cm, rounded corners},
    line/.style={draw, -latex}]

    \node [cls] (S) {Stream};
    \node [cls, below left=1cm of S] (I) {Input};
    \node [cls, below right=1cm of S] (O) {Output};
    \node [cls, below=1cm of I] (B1) {Byte};
    \node [cls, below=1cm of O] (B2) {Char};
...
\begin{itemize}
    \item \textbf{Stream}: Sequence of data flowing between source and destination
    \item \textbf{Buffering}: Improves performance by reducing disk access
\end{itemize}
\end{solutionbox}
...
\textbf{Write a java program to display the content of a text file and perform append operation on the text file.}
...
\begin{itemize}
    \item \textbf{FileWriter(file, true)}: Second parameter enables append mode
    \item \textbf{BufferedReader}: Efficiently reads text by lines
    \item \textbf{Reusable Method}: Encapsulates reading functionality
\end{itemize}
\end{solutionbox}
