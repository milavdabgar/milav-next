...
\bottomrule\noalign{}
\endlastfoot
\texttt{and} & બંને સ્ટેટમેન્ટ સાચા હોય તો True રિટર્ન કરે &
\texttt{x\ =\ 5;\ print(x\ \textgreater{}\ 3\ and\ x\ \textless{}\ 10)}
& \texttt{True} \\
\texttt{or} & બે સ્ટેટમેન્ટમાંથી એક સાચું હોય તો True રિટર્ન કરે &
\texttt{x\ =\ 5;\ print(x\ \textgreater{}\ 10\ or\ x\ ==\ 5)} &
\texttt{True} \\
\texttt{not} & પરિણામને ઉલટાવે, જો પરિણામ સાચું હોય તો False રિટર્ન કરે &
\texttt{x\ =\ 5;\ print(not(x\ \textgreater{}\ 3))} & \texttt{False} \\
\end{longtable}
}
...
has\_cash = True
print("ખરીદી કરી શકે છે:", has\_credit\_card or has\_cash)  \# True

\# લોજિકલ NOT ઉદાહરણ
is\_holiday = False
print("આજે કામ કરવું જોઈએ:", not is\_holiday)  \# True
\end{verbatim}
...
\# પરિણામો દર્શાવો
print("સાદું વ્યાજ:", round(simple\_interest, 2))
print("ચક્રવૃદ્ધિ વ્યાજ:", round(compound\_interest, 2))
\end{verbatim}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{x*y} & \texttt{16} & ગુણાકાર: 8 \times 2 = 16 \\
\texttt{x**y} & \texttt{64} & પાવર: 8^{2} = 64 \\
\texttt{x\%y} & \texttt{0} & મોડ્યુલો (શેષ): 8 \div 2 = 4 શેષ 0 \\
\texttt{x\textgreater{}y} & \texttt{True} & તુલના: 8 \textgreater{} 2 સાચું
છે \\
\end{longtable}
}
...
print("સૂત્રનો ઉપયોગ કરીને સરવાળો:", sum\_formula)
...
def main():
    ...
    if choice.lower() == {n}:
        ...
        print("ઉલટાવેલ નંબર:", reverse\_number(num))
    elif choice.lower() == {s}:
        text = input("એક સ્ટ્રિંગ દાખલ કરો: ")
        print("ઉલટાવેલ સ્ટ્રિંગ:", reverse\_string(text))
    else:
        print("અમાન્ય પસંદગી!")
...
def demonstration():
    ...
    print("ફંક્શનની અંદર {- ગ્લોબલ વેરીએબલ:"}, global\_var)
    
    \# લોકલ વેરીએબલ એક્સેસ કરવું
    print("ફંક્શનની અંદર {- લોકલ વેરીએબલ:"}, local\_var)
    
    \# ગ્લોબલ નામ ધરાવતું લોકલ વેરીએબલ બનાવવું
    global\_var = "હું ગ્લોબલ નામવાળો લોકલ છું"
    print("ફંક્શનની અંદર {- શેડોડ ગ્લોબલ:"}, global\_var)

\# ફંક્શન કૉલ
demonstration()

\# ગ્લોબલ વેરીએબલ એક્સેસ કરવું
print("ફંક્શનની બહાર {- ગ્લોબલ વેરીએબલ:"}, global\_var)

\# લોકલ વેરીએબલ એક્સેસ કરવાનો પ્રયાસ ભૂલ ઉત્પન્ન કરશે
\# print("ફંક્શનની બહાર {- લોકલ વેરીએબલ:", local\_var)  \# ભૂલ!}
\end{verbatim}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{append()} & અંતે એલિમેન્ટ ઉમેરે છે &
\texttt{fruits\ =\ [\textquotesingle{}apple\textquotesingle{}];\ fruits.append(\textquotesingle{}banana\textquotesingle{});\ print(fruits)}
&
\texttt{[\textquotesingle{}apple\textquotesingle{},\ \textquotesingle{}banana\textquotesingle{}]} \\
\texttt{insert()} & ચોક્કસ પોઝિશન પર એલિમેન્ટ ઉમેરે &
\texttt{nums\ =\ [1,\ 3];\ nums.insert(1,\ 2);\ print(nums)} &
\texttt{[1,\ 2,\ 3]} \\
\texttt{remove()} & ચોક્કસ આઈટમ દૂર કરે &
\texttt{colors\ =\ [\textquotesingle{}red\textquotesingle{},\ \textquotesingle{}blue\textquotesingle{}];\ colors.remove(\textquotesingle{}red\textquotesingle{});\ print(colors)}
& \texttt{[\textquotesingle{}blue\textquotesingle{}]} \\
\texttt{pop()} & ચોક્કસ ઇન્ડેક્સ પર આઈટમ દૂર કરે &
\texttt{letters\ =\ [\textquotesingle{}a\textquotesingle{},\ \textquotesingle{}b\textquotesingle{},\ \textquotesingle{}c\textquotesingle{}];\ x\ =\ letters.pop(1);\ print(x,\ letters)}
&
\texttt{b\ [\textquotesingle{}a\textquotesingle{},\ \textquotesingle{}c\textquotesingle{}]} \\
\texttt{clear()} & બધા એલિમેન્ટ્સ દૂર કરે &
\texttt{items\ =\ [1,\ 2];\ items.clear();\ print(items)} &
\texttt{[]} \\
\texttt{len()} & એલિમેન્ટ્સની સંખ્યા પાછી આપે &
\texttt{print(len([1,\ 2,\ 3]))} & \texttt{3} \\
\texttt{sorted()} & સૉર્ટેડ લિસ્ટ પાછી આપે &
\texttt{print(sorted([3,\ 1,\ 2]))} & \texttt{[1,\ 2,\ 3]} \\
\texttt{max()/min()} & મહત્તમ/લઘુત્તમ મૂલ્ય પાછું આપે &
\texttt{print(max([5,\ 10,\ 3]),\ min([5,\ 10,\ 3]))} &
\texttt{10\ 3} \\
\end{longtable}
}
...
\# એલિમેન્ટ્સ ઉમેરવા
my\_list.append(9)
print("append પછી:", my\_list)

my\_list.insert(2, 7)
print("insert પછી:", my\_list)

\# એલિમેન્ટ્સ દૂર કરવા
my\_list.remove(1)  \# પ્રથમ 1 દૂર કરે છે
print("remove પછી:", my\_list)

popped = my\_list.pop()  \# છેલ્લું એલિમેન્ટ દૂર કરે અને પાછું આપે
print("pop કરેલું મૂલ્ય:", popped)
print("pop પછી:", my\_list)

\# અન્ય ઓપરેશન્સ
print("લંબાઈ:", len(my\_list))
print("સૉર્ટેડ:", sorted(my\_list))
print("સરવાળો:", sum(my\_list))
print("1 ની સંખ્યા:", my\_list.count(1))
\end{verbatim}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{abs()} & નિરપેક્ષ મૂલ્ય પાછું આપે & \texttt{abs(-5)} \rightarrow \texttt{5} \\
\texttt{round()} & નજીકના પૂર્ણાંક સુધી ગોળ કરે & \texttt{round(3.7)} \rightarrow
\texttt{4} \\
\texttt{max()} & સૌથી મોટી આઈટમ પાછી આપે & \texttt{max(1,\ 5,\ 3)} \rightarrow
\texttt{5} \\
\texttt{min()} & સૌથી નાની આઈટમ પાછી આપે & \texttt{min(1,\ 5,\ 3)} \rightarrow
\texttt{1} \\
\texttt{sum()} & ઇટરેબલની આઈટમ્સનો સરવાળો કરે &
\texttt{sum([1,\ 2,\ 3])} \rightarrow \texttt{6} \\
\texttt{pow()} & x ને y ની ઘાત પાછી આપે & \texttt{pow(2,\ 3)} \rightarrow
\texttt{8} \\
\texttt{divmod()} & ભાગફળ અને શેષ પાછા આપે & \texttt{divmod(7,\ 2)} \rightarrow
\texttt{(3,\ 1)} \\
\end{longtable}
}
...
\tightlist
\item
  \texttt{math.sqrt()}: વર્ગમૂળ
\item
  \texttt{math.floor()}: નીચે ગોળ કરે
\item
  \texttt{math.ceil()}: ઉપર ગોળ કરે
\item
  \texttt{math.factorial()}: ફેક્ટોરિયલ
\item
  \texttt{math.gcd()}: મહત્તમ સામાન્ય અવયવ
\end{itemize}
...
sub\_string = input("શોધવાની સબસ્ટ્રિંગ દાખલ કરો: ")

\# તપાસો અને પરિણામ દર્શાવો
if check\_substring(main\_string, sub\_string):
    print(f"{}\{sub\_string\}{ }\{main\_string\}{ માં હાજર છે"})
else:
    print(f"{}\{sub\_string\}{ }\{main\_string\}{ માં હાજર નથી"})
\end{verbatim}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{બનાવટ} & કૌંસ સાથે ટપલ બનાવવું & \texttt{t\ =\ (1,\ 2,\ 3)} &
\texttt{(1,\ 2,\ 3)} \\
\textbf{ઇન્ડેક્સિંગ} & ટપલ એલિમેન્ટ્સ એક્સેસ કરવા & \texttt{t[1]} &
\texttt{2} \\
\textbf{સ્લાઇસિંગ} & ટપલનો સબસેટ મેળવવો & \texttt{t[1:3]} &
\texttt{(2,\ 3)} \\
\textbf{કેટેનેશન} & બે ટપલ જોડવા & \texttt{(1,\ 2)\ +\ (3,\ 4)} &
\texttt{(1,\ 2,\ 3,\ 4)} \\
\textbf{રિપિટેશન} & ટપલ એલિમેન્ટ્સ રિપીટ કરવા & \texttt{(1,\ 2)\ *\ 2} &
\texttt{(1,\ 2,\ 1,\ 2)} \\
\textbf{મેમ્બરશિપ} & એલિમેન્ટ છે કે નહીં તે તપાસવું & \texttt{3\ in\ (1,\ 2,\ 3)}
& \texttt{True} \\
\textbf{len()} & આઇટમ્સની સંખ્યા મેળવવી & \texttt{len((1,\ 2,\ 3))} &
\texttt{3} \\
\textbf{min()/max()} & લઘુત્તમ/મહત્તમ મૂલ્ય શોધવું & \texttt{min((3,\ 1,\ 2))}
& \texttt{1} \\
\textbf{count()} & મૂલ્યની સંખ્યા ગણવી & \texttt{(1,\ 2,\ 1).count(1)} &
\texttt{2} \\
\textbf{index()} & મૂલ્યની પોઝિશન શોધવી & \texttt{(1,\ 2,\ 3).index(2)} &
\texttt{1} \\
\textbf{sorted()} & ટપલમાંથી સૉર્ટેડ લિસ્ટ પાછી આપે &
\texttt{sorted((3,\ 1,\ 2))} & \texttt{[1,\ 2,\ 3]} \\
\end{longtable}
}
...
print("મૂળ ટપલ:", my\_tuple)

\# એલિમેન્ટ્સ એક્સેસ કરવા
print("પ્રથમ એલિમેન્ટ:", my\_tuple[0])
print("છેલ્લું એલિમેન્ટ:", my\_tuple[{-}1])
print("સ્લાઇસ (1:4):", my\_tuple[1:4])
...
\# ફંક્શન્સ અને મેથડ્સ
print("લંબાઈ:", len(my\_tuple))
print("1 ની સંખ્યા:", my\_tuple.count(1))
print("4 નો ઇન્ડેક્સ:", my\_tuple.index(4))
print("ન્યૂનતમ મૂલ્ય:", min(my\_tuple))
print("મહત્તમ મૂલ્ય:", max(my\_tuple))
print("સૉર્ટેડ:", sorted(my\_tuple))  \# લિસ્ટ પાછી આપે
...
print("સિમેટ્રિક ડિફરન્સ:", set1 \^{} set2)  \# વૈકલ્પિક: set1.symmetric\_difference(set2)
