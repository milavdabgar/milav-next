...
JZ ZERO\_FOUND       ; A zero છે તો jump કરો
...
ORG 0000H
    ...
    CALL DISPLAY\_MSG    ; Message display કરો
    SJMP $              ; અહીં stop કરો
...
NEXT\_CHAR:
    ...
    CALL SEND\_CHAR      ; LCD પર character send કરો
...
\begin{verbatim}
ORG 0000H
    MOV DPTR,\#DIGIT\_TABLE   ; Lookup table તરફ point કરો
...
MAIN\_LOOP:
    ...
    INC R0                  ; Next digit
    CJNE R0,\#10,MAIN\_LOOP   ; 10 reached છે કે check કરો
    MOV R0,\#0               ; 0 પર reset કરો
    SJMP MAIN\_LOOP          ; Repeat કરો
...
ORG 0000H
MAIN:
    CALL READ\_TEMP      ; ADC માંથી temperature read કરો
    CALL DISPLAY\_TEMP   ; Display પર temperature show કરો
    CALL TEMP\_CONTROL   ; Heating/cooling control કરો
...
TEMP\_CONTROL:
    CJNE A,\#30,CHECK\_HIGH   ; Setpoint (30^) સાથે compare કરો
CHECK\_HIGH:
    JC TEMP\_LOW             ; A { 30 હોય તો temperature low છે}
