...
\texttt{AVG(column\_name)} - કોલમમાં મૂલ્યોની સરેરાશ ગણતરી કરે છે\\
\texttt{SELECT\ AVG(salary)\ FROM\ Employee;} - સરેરાશ પગાર આપે છે
...
\texttt{ROUND(number,\ decimal\_places)} - સંખ્યાને નિર્દિષ્ટ દશાંશ સ્થાનો સુધી
રાઉન્ડ કરે છે\\
\texttt{SELECT\ ROUND(125.679,\ 2)\ FROM\ DUAL;} - 125.68 આપે છે
