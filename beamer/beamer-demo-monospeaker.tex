...
% --- Section Popup ---
% \AtBeginSection removed to allow explicit parsing of notes
...
\begin{frame}
    ...
    \note{
        [click] Welcome to the Comprehensive Beamer Feature Showcase. This document covers not just content, but structure, navigation, and visual styling logic.
    }
\end{frame}
...
\begin{frame}{Table of Contents}
    \tableofcontents
    \note{
        A table of contents is crucial for longer presentations. Beamer generates this automatically from section commands.
    }
\end{frame}
...
\begin{frame}
  ...
  \begin{beamercolorbox}[sep=8pt,center,shadow=true,rounded=true]{title}
    \usebeamerfont{title}Structure \& Layout\par%
  \end{beamercolorbox}
  \vfill
  \note{[click] Section: Structure and Layout}
\end{frame}
...
\begin{frame}{Incremental Reveals}
    ...
    \note{
        In our first section, Structure and Layout. We start with incremental reveals to control pacing.
        [click] Step two.
        [click] And step three.
    }
\end{frame}
...
def fib(n):
    ...
    \note{
        Columns are essential for comparisons.
    }
\end{frame}
...
\begin{frame}{Beamer Blocks}
    ...
    \note{
        Beamer provides distinct block types. The standard block.
        [click] The alert block for emphasis.
        [click] And the examples block.
    }
\end{frame}
...
\begin{frame}
  ...
  \centering
  \begin{beamercolorbox}[sep=8pt,center,shadow=true,rounded=true]{title}
    \usebeamerfont{title}Rich Content\par%
  \end{beamercolorbox}
  \vfill
  \note{[click] Section: Rich Content}
\end{frame}
...
\begin{frame}{Math \& Equations}
    ...
    \note{
        Moving to Rich Content. Math support is first class.
        [click] We can reveal derivations step by step.
        [click] Notice the clean alignment.
    }
\end{frame}
...
def calculate_area(radius):
    ...
    \note{
        Code evolution is crucial for technical talks.
        [click] We update the code in place using overlays.
    }
\end{frame}
...
\begin{frame}{Styled Components}
    ...
    \note{
        We can also create custom components like Info Cards.
        [click] Or Alert Boxes.
    }
\end{frame}
...
\begin{frame}
  ...
  \centering
  \begin{beamercolorbox}[sep=8pt,center,shadow=true,rounded=true]{title}
    \usebeamerfont{title}Visuals \& Navigation\par%
  \end{beamercolorbox}
  \vfill
  \note{[click] Section: Visuals and Navigation}
\end{frame}
...
\begin{frame}{Digrams (TikZ)}
    ...
    \note{
        Vector diagrams are rendered natively.
        [click] And can be highlighted dynamically.
    }
\end{frame}
...
\begin{frame}{Navigation Buttons}
    ...
    \note{
        Interactive elements like buttons allow for non-linear presentations. Typically less used in linear video, but valuable for reference.
    }
\end{frame}
...
\begin{frame}
  ...
  \centering
  \begin{beamercolorbox}[sep=8pt,center,shadow=true,rounded=true]{title}
    \usebeamerfont{title}Animation\par%
  \end{beamercolorbox}
  \vfill
  \note{[click] Section: Animation}
\end{frame}
...
\begin{frame}[label=transitions]{Transitions}
    ...
    \note{
        Finally, animation.
        Use transitions sparingly, but they add polish.
    }
\end{frame}
