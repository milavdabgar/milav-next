...
``` {.java language="Java" caption="ત્રણ Numbers માંથી Maximum શોધો"}
...
RC low-pass filter માટે **cutoff frequency** નો formula છે: $$f_c = \frac{1}{2\pi RC}$$

મૂલ્યો મૂકીએ: $$f_c = \frac{1}{2\pi \times 1500 \times 100 \times 10^{-9}}$$ $$f_c = \frac{1}{2\pi \times 1.5 \times 10^{-4}}$$ $$f_c = \frac{1}{9.42 \times 10^{-4}} = 1061.57\,Hz \approx 1.06\,kHz$$

##### પગલું 2: Cutoff પર Output Voltage

Cutoff frequency પર, output voltage એ input voltage ના **0.707 ગણા** (અથવા $\frac{1}{\sqrt{2}}$) હોય છે: $$V_{out} = 0.707 \times V_{in} = 0.707 \times 10 = 7.07\,V$$
...
:   Integrated circuits જેવા કે `555 timer` (oscillator), op-amps (amplifier).
...
:   જ્યારે input AC positive હોય, diode forward-biased (conduct) થાય છે. Current load resistor $R_L$ માંથી વહે છે, output voltage ઉત્પન્ન કરે છે.

Negative Half-Cycle:

:   જ્યારે input AC negative હોય, diode reverse-biased (block) થાય છે. કોઈ current વહેતું નથી, output voltage શૂન્ય હોય છે.

પરિણામ:

:   Output પર ફક્ત positive half-cycles દેખાય છે, pulsating DC બનાવે છે.
...
Half-wave rectifiers નો ઉપયોગ low-power applications માં થાય છે જેવા કે battery charging, signal demodulation, અને voltage multipliers. તેઓ poor efficiency ને કારણે high-power applications માટે *યોગ્ય નથી*.
