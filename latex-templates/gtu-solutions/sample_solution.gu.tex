...
ત્રણ numbers માંથી \textbf{maximum} શોધવા માટે, અમે values ની સરખામણી કરવા માટે \textbf{conditional statements} (if-else) નો ઉપયોગ કરીએ છીએ. Program ત્રણ numbers input તરીકે લે છે અને તેમાંથી ``\emph{સૌથી મોટી value}રત કરે છે.
...
\begin{description}
    \item[Logic:] પ્રથમ \texttt{a} અને \texttt{b} ની સરખામણી કરો, મોટી value ને \texttt{max} માં store કરો
    \item[બીજી સરખામણી:] અંતિમ maximum મેળવવા માટે \texttt{max} ને \texttt{c} સાથે સરખાવો
    \item[વૈકલ્પિક:] Concise code માટે \texttt{Math.max(a, Math.max(b, c))} નો ઉપયોગ કરી શકાય
\end{description}
...
\begin{description}
    ...
    \item[Output Voltage:] $V_{out} = 7.07\,V$ cutoff પર
    \item[Attenuation:] $-3\,dB$ cutoff frequency પર
    \item[Phase Shift:] $-45^\circ$ cutoff frequency પર
\end{description}

\paragraph{મેમરી ટ્રીક:}
\emph{RC-Formula: fc = 1/(2$\pi$ RC), Vout = 0.707 Vin at fc}
...
\begin{itemize}
    ...
    \item \textbf{ICs}: Integrated circuits જેવા કે \texttt{555 timer} (oscillator), op-amps (amplifier).
